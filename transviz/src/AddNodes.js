...
// import { SET_COMPONENT_NODES } from "./actions";
import { setComponentNodes } from "./canvasReducer";
...
const AddNodes = ({ nodesData, node }) => {
  ...
  const categorizeVectorStores = (nodes, accordianCategories, isFilter) => {
    ...
    if (deprecatingNodes.length) {
      ...
      accordianCategories["Vector Stores;DEPRECATING"] = isFilter
        ? true
        : false;
    }
    if (newNodes.length) {
  ...
  const getSearchedNodes = (value) => {
    const passed = nodesData.filter((nd) => {
      ...
      const passesCategory = nd.category
        .toLowerCase()
  ...
  useEffect(() => {
    if (nodesData) {
      ...
      dispatch(setComponentNodes(nodesData));
    }
